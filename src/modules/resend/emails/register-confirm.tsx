...
function RegisterConfirmEmailComponent({ activateToken, first_name }: RegisterConfirmEmailProps) {
  const href = `${STORE_URL}/activate?token=${activateToken}`
  const greeting = first_name ? `Hey ${first_name}` : "Hey"

  return (
    <Tailwind>
      <Html style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
        <Head />
        <Preview>One click and you're in — confirm your Onyx Genetics registration</Preview>
          ...
          <Container style={{ maxWidth: 480, margin: "0 auto" }}>
            ...
            <Section style={{ background: "#111111", padding: "28px 24px", border: "1px solid #1f1f1f", borderTop: "none" }}>
              ...
              <Text style={{ color: "#9ca3af", fontSize: 14, margin: "0 0 20px", lineHeight: 1.6 }}>
                One click and you're in. Your account will be created and any orders you've placed will be attached automatically — no password needed.
              </Text>
...
export const registerConfirmEmail = (props: RegisterConfirmEmailProps) => (
  <RegisterConfirmEmailComponent {...props} />
)

export default RegisterConfirmEmailComponent
